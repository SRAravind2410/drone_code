...
void NavEKF3_core::FuseVelPosNED()
{
    ...
    if (fuseVelData || fusePosData || fuseHgtData) {
        ...
        if (gpsDataDelayed.have_vz && fuseVelData && (frontend->sources.getPosZSource(core_index) != AP_NavEKF_Source::SourceZ::GPS)) {
        ...
        if (fuseVelData) {
            ...
            if ((!frontend->sources.haveVelZSource(core_index) || PV_AidingMode != AID_ABSOLUTE ||
