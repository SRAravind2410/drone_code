...
/// set_speed_NE - allows main code to pass target horizontal velocity for wp navigation
void AC_WPNav::set_speed_NE(float speed_cms)
{
...
/// get_wp_bearing_to_destination_cdeg - get bearing to next waypoint in centi-degrees
int32_t AC_WPNav::get_wp_bearing_to_destination_cdeg() const
{
...
bool AC_WPNav::get_vector_NEU_cm(const Location &loc, Vector3f &pos_from_origin_neu_cm, bool &terrain_alt)
{
    ...
    Vector2f pos_from_origin_NE_cm;
    if (!loc.get_vector_xy_from_origin_NE(pos_from_origin_NE_cm)) {
        return false;
    }
    ...
    pos_from_origin_neu_cm.x = pos_from_origin_NE_cm.x;
    pos_from_origin_neu_cm.y = pos_from_origin_NE_cm.y;
