...
void AC_Circle::set_center(const Location& center)
{
    if (center.get_alt_frame() == Location::AltFrame::ABOVE_TERRAIN) {
        ...
        int32_t terr_alt_cm;
        if (center.get_vector_xy_from_origin_NE(center_ne_cm) && center.get_alt_cm(Location::AltFrame::ABOVE_TERRAIN, terr_alt_cm)) {
        ...
        if (!center.get_vector_from_origin_NEU(circle_center_neu_cm)) {
