...
const AP_Param::Info Plane::var_info[] = {
    ...
    // @Description: When enabled, this adds user stick input to the control surfaces in auto modes, allowing the user to have some degree of flight control without changing modes. There are 3 types of stick mixing available. If you set STICK_MIXING to 1 or 2 then it will use "fly by wire" mixing. 1 will only provide roll control, while 2 also provides FBW-A style pitch control. If you set STICK_MIXING to 3 then it will apply to the yaw while in quadplane modes only, such as while doing an automatic VTOL takeoff or landing. WARNING: FBW-A pitch control does not offer flight envelope protections. Prolonged pitch inputs in mode 2 can result in a stall or overspeed condition, and should be avoided.
    // @Values: 0:Disabled,1:FBW style (roll only), 2:FBW-A style,3:VTOL Yaw only
    // @User: Advanced
    GSCALAR(stick_mixing,           "STICK_MIXING",   uint8_t(StickMixing::FBW_ROLL)),
